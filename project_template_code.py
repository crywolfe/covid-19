...
def meta_data(df):
    try:
        ...
        return pd.concat([total, percent, unique, datatypes], axis=1, keys=['Total', 'Percent', 'Unique', \
                                                                            'Data_Type']).sort_values(by="Percent",
                                                                                                      ascending=False)
    except Exception as e:
...
def plot_charts(var, defaulters, nondefaulters, label_rotation, horizontal_layout):
    ...
    s2 = sns.countplot(ax=ax2, x=nondefaulters[var], data=nondefaulters,
                       order=nondefaulters[var].value_counts().index, )
    if (label_rotation):
...
def voting_ensemble_classification(df, logreg, svc, knn, gaussian, perceptron, linearsvc, sgd, decisiontree,
                                   randomforest, clf, AB, GBC, ETC, xgbs):
    try:
        array = df.values
        X = array[:, 0:8]
        Y = array[:, 8]
...
# -------------------------------------------- model evaluation-------------------------------------------#
def model_evaluation(results_linreg, results_logreg, results_svc, results_knn, results_gaussian, results_perceptron,
                     results_linearsvc, results_sgd, results_decisiontree, results_randomforest, results_clf,
                     results_AB, results_GBC, results_ETC, results_xgbs, results_ensemble, results_neural_network):
    try:
        # Model evaluation
        ...
        models = pd.DataFrame({
            'Model': ['Linear Regression', 'Support Vector Machines', 'KNN', 'Logistic Regression',
                      ...
                      'Gradient Boosting Classifier', 'Extra TreesClassifier', 'XGB Classifier', 'Ensemble',
                      'Neural Network'],
            'Score': [results_linreg.mean(), results_logreg.mean(), results_svc.mean(), results_knn.mean(),
                      results_gaussian.mean(), results_perceptron.mean(), results_linearsvc.mean(), results_sgd.mean(),
                      results_decisiontree.mean(), results_randomforest.mean(), results_clf.mean(), results_AB.mean(),
                      results_GBC.mean(), results_ETC.mean(), results_xgbs.mean(), results_ensemble.mean(),
